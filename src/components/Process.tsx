  ...
  <section className="py-20 bg-soft" aria-label="Onze werkwijze">
    <div className="container-x">
      ...
      <motion.div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-5" variants={staggerContainer} initial="hidden" whileInView="show" viewport={viewport}>
        {steps.map((s) => (
          <motion.div key={s.nr} variants={staggerItem} className="relative rounded-2xl border bg-card p-6 shadow-soft overflow-hidden group hover:-translate-y-0.5 hover:shadow-card transition-all duration-300">
            {/* accent left bar */}
            <div className="absolute left-0 inset-y-0 w-[3px] rounded-l-2xl" style={{ background: "linear-gradient(180deg, hsl(214 82% 56%) 0%, hsl(200 88% 65%) 100%)" }} />
            <div
              className="mb-4 inline-flex h-10 w-10 items-center justify-center rounded-xl text-sm font-black text-white"
              style={{ background: "linear-gradient(135deg, hsl(214 82% 54%) 0%, hsl(220 85% 46%) 100%)" }}
            >
              {s.nr}
            </div>
